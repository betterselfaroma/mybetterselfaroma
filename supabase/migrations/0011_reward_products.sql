-- Reward products and redemption records.
-- Safe to run in Supabase SQL Editor: no drops, no data deletion.
...
-- Keep compatibility with existing admin/mobile-admin policies and RPCs.
...
set search_path = public
as $$
  select public.is_admin_or_staff(p_user_id)
$$;

grant execute on function public.current_customer_id() to anon, authenticated, service_role;
grant execute on function public.is_admin_or_staff(uuid) to anon, authenticated, service_role;
...
-- ---------------------------------------------------------------------------
-- Reward redemptions.
-- The product_id/points_cost columns are the new product-redemption path.
-- reward_id/points_used/admin_notes are kept for compatibility with the old
-- rewards catalogue and existing admin pages. No old data is removed.
-- ---------------------------------------------------------------------------
...
-- Required supporting tables for points/audit writes used by Admin flows.
...
-- ---------------------------------------------------------------------------
-- Optional image bucket used by the web Admin reward product uploader.
-- ---------------------------------------------------------------------------
...
create policy reward_products_staff_insert on storage.objects
  for insert
  with check (bucket_id = 'reward-products' and public.is_admin_or_staff());
...
create policy reward_products_staff_update on storage.objects
  for update
  using (bucket_id = 'reward-products' and public.is_admin_or_staff())
  with check (bucket_id = 'reward-products' and public.is_admin_or_staff());
...
grant insert, update, delete on public.reward_products to authenticated;
...
create policy reward_redemptions_self_select on public.reward_redemptions
  for select
  using (customer_id = public.current_customer_id() or public.is_admin_or_staff());
...
-- ---------------------------------------------------------------------------
-- Atomic member redemption used by /member/rewards.
-- ---------------------------------------------------------------------------
